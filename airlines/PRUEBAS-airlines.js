...
function welcome() {
    ...
    console.log('El coste medio de los vuelos es: '+(suma/10)+'€');
...
//Primero hago un condicional de que si el nombre de usuario introducido es Lola, el programa se comporte en modo admin. Si no, en modo user
    if (nombreUsuario === 'LOLA') {
        console.log('Hola LOLA. Eres admin, así que puedes añadir hasta 15 vuelos y eliminar vuelos mediante su ID.');
        admin();
    } else {
        user();
    }
...
//Luego le pregunto que quiere exactamente filtrar
        let orden = prompt('Escribe, ALTO para ver vuelos con precios más altos. BAJO para ver vuelos con precios más bajos. IGUAL para ver los del mismo precio.').toUpperCase();
//Creo un bucle que itere por el array y vaya comparando los cost con lo que el cliente eligió y finalmente lo pase por consola. 
        for (let i = 0; i < flights.length; i++) {
            if (orden === 'ALTO') {
